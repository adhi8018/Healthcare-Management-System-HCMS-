<!-- dbcon_pat.php - for patient dashboard db connection -->
<?php
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['updatePatient'])) {
    ...
    $fetch_query = "SELECT patient_password FROM patient WHERE patient_id='$patient_id'";
    $fetch_result = mysqli_query($conn, $fetch_query);
    $row = mysqli_fetch_assoc($fetch_result);
    $existing_password = $row['patient_password']; // Keep old password
    ...
    $update_query = "UPDATE patient SET 
        ...
        patient_password='$hashed_pass' 
        WHERE patient_id='$patient_id'";

    $update_result = mysqli_query($conn, $update_query);

    if ($update_result) {
        echo "<script>alert('Patient details updated successfully!'); window.location.href='pro_pat.php';</script>";
    } else {
